...
export const CLIPFLY_BASE = "https://www.clipfly.ai";
export const GEMINIGEN_BASE = "https://api.geminigen.ai";
...
export async function retryPost<T>(
  ...
  for (let i = 0; i < retries; i++) {
    try {
      const res = await axios.post<T>(url, data, { ...config, timeout: 30000, validateStatus: () => true });
      return res.data;
    } catch (err) {
      lastErr = err;
...
export async function retryGet<T>(
  ...
  for (let i = 0; i < retries; i++) {
    try {
      const res = await axios.get<T>(url, { ...config, timeout: 15000, validateStatus: () => true });
      return res.data;
    } catch (err) {
      lastErr = err;
      await new Promise((r) => setTimeout(r, delayMs));
